...
def process_link(choice, link, name_template, total_eps):
    ...
    try:
        ...
        c_h2_divs = dom.xpath(
            '//div[contains(@class, "c_h2") or contains(@class, "c_h2b")]'
        )
        ...
        if choice == "Biggest Size":
            c_h2_divs.sort(
                key=lambda div: convert_size(div.xpath(".//b/text()")[1])
                if len(div.xpath(".//b/text()")) > 1
                else 0
            )
        elif choice == "Most Downloaded":
            c_h2_divs.sort(
                key=lambda div: int(div.xpath(".//b/text()")[2])
                if len(div.xpath(".//b/text()")) > 2
                else 0
            )
        else:
            c_h2_divs.sort(
                key=lambda div: convert_date(div.xpath(".//b/text()")[4])
                if len(div.xpath(".//b/text()")) > 4
                else datetime.min
            )
        ...
        type = link.split("/")[-2]
        dstatus = "Success"
        if "tokyoinsider" not in download_link:
            download_link = f"No valid link found {type}: {epn}"
            dstatus = "Faild no valid link"
...
def fetch_download(choice, links, name_template, total_eps):
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:  # Use 5 threads
        executor.map(
            lambda link: process_link(choice, link, name_template, total_eps), links
        )
...
def append_links(epsRange, episodes, links):
    for i in range(int(epsRange[0]), int(epsRange[1]) + 1):
        links.append(
            "https://www.tokyoinsider.com" + episodes[i * -1].xpath("./@href")[0]
        )
...
def okay(URL):
    ...
    for key in types.keys():
        ...
        length = len(val)
        if length > 0:
            typer_input = typer.prompt(
                f"{length} {types[key]} found - select a range to download (0: None)",
                default=f"1-{length}",
            )
            if typer_input != "0":
    ...
    selected = inquirer.list_input(
        ...
        choices=["Biggest Size", "Most Downloaded", "Latest"],
    )
    ...
    print(">>> fetching...")
    fetch_download(
        selected, links, name_template, total_eps
    )  # Run multi-threaded download fetcher


def custom_names():
    print("Default custom name: <name> - <type><number> [uploader]")
    name_template = input(
        "Write your own? Press Enter for default.\nAvailable arguments: {anime_name}, {type}, {episode_number}, {size}, {uploader}, {upload_date}: "
    )

    if name_template != "" and "{episode_number}" not in name_template:
        name_template = input(
            "Without the episode number, the filenames won't be unique. Press Enter for default. "
        )
...
def format_names(
    name_template, anime_name, type, epn, b_elements, total_eps, file_extension
):
    full_epn = epn
...
def cli_main(
    url: str = typer.Option(
        "https://www.tokyoinsider.com/anime/B/Bleach_(TV)", prompt=True
    ),
):
    okay(url)
